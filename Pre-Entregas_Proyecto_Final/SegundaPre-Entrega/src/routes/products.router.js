...
router.get("/", async (req, res) => {
  try {
    const { limit = 10, page = 1, sort, query } = req.query; //Desestructuración del objeto req
...
router.put("/:pid", async (req, res) => {
  const id = req.params.pid;
  const productoActualizando = req.body;
  console.log("Testeando esta vaina:", productoActualizando);
  try {
